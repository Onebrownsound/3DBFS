...
class Galaxy():  # Creates an instance of NxNxN matrix and populates it with asteroids and gravity wells
    def __init__(self, n, start,
                 end):  # all the properties for the space graph are created her on a per 'instance' basis
        self.n = n
        self.size = n * n * n
        self.asteroidspop = math.floor(self.size * 0.10)
        self.gravitywellspop = math.floor(self.size * 0.10)
        self.matrix = [[["." for x in range(n)] for x in range(n)] for x in
                       range(n)]  # Let it be noted a "." represents a non-obstacle and non start or end node.
        self.start = start
        self.end = end
    ...
    def createobstacles(self):
        ...
        for item in range(self.asteroidspop):  # asteroid creation
            ...
            while self.matrix[x][y][z] in ['A', 'S',
                                           'E']:  # after a random coordinate is chosen it makes sure its not already a create obstacle if it is it rerolls that random
                x, y, z = random.randint(0, self.n - 1), random.randint(0, self.n - 1), random.randint(0, self.n - 1)
            self.matrix[x][y][z] = 'A'

        for item in range(self.gravitywellspop):  # gravity well creation
            x, y, z = random.randint(0, self.n - 1), random.randint(0, self.n - 1), random.randint(0,
                                                                                                   self.n - 1)  # same logic applies from before
...
def custombfs(graph, start):
    ...
    while todo:
        ...
        for move in moves:  # iterates through the bank of "moves"
            if not (0 <= pos[0] + move[0] < length and 0 <= pos[1] + move[1] < length and 0 <= pos[2] + move[
                2] < length):  # cycles through moves list and decides whether legal or illegal
                continue
...
def main():
    ...
    tempmaster = deepcopy(
        Space.matrix)  # ran into problems converting the space object to have the node "properties" so I had to make a deep copy of the object and work with that

    for q in range(Space.n):
        for r in range(Space.n):
            for z in range(Space.n):
                tempmaster[q][r][z] = Node(tempmaster[q][r][z], (
                    q, r, z))  # converts all coordinates to nodes, is inefficient but works none the less

    solution = custombfs(tempmaster, tempmaster[Space.starta][Space.startb][Space.startc])
    print(
        solution)  # prints solutions. in the even no solution is found custombfs with raise an exception and say path not found
